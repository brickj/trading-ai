...
class EnhancedTradingStrategy(TradingStrategy):
    ...
    def _generate_hold_recommendations(self, symbol: str, current_price: float,
                                    sentiment_score: float, confidence: float, bias: str) -> List[Dict]:
    ...
    def _generate_conservative_recommendation(self, symbol: str, current_price: float, 
                                           sentiment_score: float, confidence: float, action: str) -> Dict:
    ...
    def _generate_aggressive_recommendation(self, symbol: str, current_price: float, 
                                          sentiment_score: float, confidence: float, action: str) -> Dict:
    ...
    def _generate_moderate_recommendation(self, symbol: str, current_price: float, 
                                        sentiment_score: float, confidence: float, action: str) -> Dict:
    ...
    def _generate_income_recommendation(self, symbol: str, current_price: float, 
                                      sentiment_score: float, confidence: float, action: str) -> Dict:
    ...
    def _generate_momentum_recommendation(self, symbol: str, current_price: float, 
                                        sentiment_score: float, confidence: float, action: str) -> Dict:
    ...
    def test_recommendations_against_historical_data(self, recommendations: List[Dict], 
        ...
        for rec in recommendations:
            ...
            if isinstance(historical_data, pd.DataFrame):
                historical_performance = self._backtest_recommendation(rec, historical_data)
            ...
            if db_performance and db_performance.get('total_with_outcomes', 0) > 0:
                ...
                matching_types = [
                    r for r in db_performance.get('recommendation_performance', [])
                    if r.get('recommendation_type') == rec.get('recommendation_type') and
                    r.get('action') == rec.get('action')
                ]
                
                if matching_types and matching_types[0].get('count', 0) >= 5:  # Need at least 5 samples
                    db_win_rate = matching_types[0].get('win_rate', 0)
                    # Adjust confidence based on past performance (weight increases with more data)
                    count = min(matching_types[0].get('count', 0), 100)  # Cap at 100 for weighting
                    weight_factor = min(0.5, count / 200)  # Max 0.5 weight for DB performance
                    ...
                    print(f"📊 Adjusted confidence for {rec['recommendation_type']} using DB history: " +
                                                    f"{rec['historical_confidence']:.3f} → {final_confidence:.3f}")
                else:
                    # Calculate final confidence (weighted average)
                    final_confidence = (rec['base_confidence'] * 0.3 + rec['historical_confidence'] * 0.7)
                    rec['confidence'] = round(final_confidence, 3)
            else:
                # Calculate final confidence (weighted average)
                final_confidence = (rec['base_confidence'] * 0.3 + rec['historical_confidence'] * 0.7)
    ...
    def _get_alpha_vantage_historical_data(self, symbol: str, days: int) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Got {len(df)} days of Alpha Vantage historical data for {symbol}")
                return df
    ...
    def _backtest_recommendation(self, recommendation: Dict, historical_data: pd.DataFrame) -> Dict:
        """
        ...
        strike_price_factor = recommendation['strike_price'] / recommendation['current_price']
        ...
        for i in range(len(historical_data) - days_to_expiry):
            ...
            for j in range(i + 1, exit_idx + 1):
                ...
                profit_loss = (current_option_price - option_entry_price) / option_entry_price
        ...
        historical_confidence = base_historical_confidence * trade_count_factor * consistency_factor
    ...
    def get_top_recommendation_with_confidence(self, symbol: str, current_price: float, 
                                             sentiment_data: Dict, signal_data: Dict) -> Dict:
        """
        ...
        tested_recommendations = self.test_recommendations_against_historical_data(recommendations)
        
        # Return the top recommendation with all alternatives
        top_recommendation = tested_recommendations[0] if tested_recommendations else None
    ...
    def generate_stock_recommendations(self, symbol: str, current_price: float, 
                                     sentiment_data: Dict, signal_data: Dict) -> List[Dict]:
        """
        ...
        if sentiment_score > 0.1:  # Bullish
            ...
            if sentiment_score > 0.5:
                aggressive_stock = {
                    ...
                    'base_confidence': base_confidence,
                    'confidence': base_confidence * 0.75,  # More aggressive = lower confidence
                    'risk_level': 'High',
    ...
    def get_comprehensive_recommendations(self, symbol: str, current_price: float, 
                                       sentiment_data: Dict, signal_data: Dict) -> Dict:
        """
        ...
        enhanced_recommendations = self.test_recommendations_against_historical_data(options_recommendations)
        ...
        enhanced_stock_recommendations = self.test_stock_recommendations_against_historical_data(stock_recommendations)
        ...
        top_stock = next((rec for rec in all_recommendations if rec['category'] == 'Stock'), None)
        top_options = next((rec for rec in all_recommendations if rec['category'] == 'Options'), None)
        ...
        return {
            ...
            'stock_recommendations': [rec for rec in all_recommendations if rec['category'] == 'Stock'],
            'options_recommendations': [rec for rec in all_recommendations if rec['category'] == 'Options'],
            'recommendation_summary': {
                'total_strategies': len(all_recommendations),
                'best_category': top_recommendation['category'] if top_recommendation else 'None',
                'best_confidence': top_recommendation.get('confidence', 0) if top_recommendation else 0,
                'stock_vs_options_winner': top_recommendation['category'] if top_recommendation else 'None'
            },
            'analysis_timestamp': datetime.now().isoformat()
        }

    def test_stock_recommendations_against_historical_data(self, recommendations: List[Dict]) -> List[Dict]:
        """
        ...
        print(f"✅ Testing {len(recommendations)} stock recommendations against {len(historical_data)} days of data")
        
        # Test each recommendation
        for rec in recommendations:
            if isinstance(historical_data, pd.DataFrame):
                backtest_result = self._backtest_stock_recommendation(rec, historical_data)
                ...
                rec['confidence'] = (historical_confidence * 0.7) + (base_confidence * 0.3)
    ...
    def _backtest_stock_recommendation(self, recommendation: Dict, historical_data: pd.DataFrame) -> Dict:
        """
        ...
        for i in range(len(historical_data) - 30):  # Need at least 30 days to evaluate
            ...
            for j in range(i + 1, min(i + 31, len(historical_data))):  # Max 30 days holding
                current_price = historical_data.iloc[j]['Close']
                ...
                if action in ['BUY', 'BUY_STOCK']:
                    ...
                    if price_change >= target_gain:
                        # Hit target
                        trades.append({'return': target_gain, 'days_held': j - i, 'exit_reason': 'target'})
                        break
                    elif price_change <= -stop_loss:
                        # Hit stop loss
                        trades.append({'return': -stop_loss, 'days_held': j - i, 'exit_reason': 'stop'})
                        break
                    elif j == min(i + 30, len(historical_data) - 1):
                        # Time exit
                        trades.append({'return': price_change, 'days_held': j - i, 'exit_reason': 'time'})
        ...
        if trades:
            ...
            return_factor = max(0, min(1, (stats['avg_return'] + 20) / 40))  # Normalize around 0-40% returns
            confidence = (win_rate_factor * 0.6) + (return_factor * 0.4)
            
        else:
            stats = {'total_trades': 0, 'win_rate': 0, 'avg_return': 0, 'max_gain': 0, 'max_loss': 0}
            confidence = recommendation.get('base_confidence', 0.5)
...
class OptionsStrategy:
    ...
    def calculate_option_price_estimate(self, stock_price: float, strike_price: float, 
    ...
    def generate_trade_signal(self, symbol: str, current_price: float, 
        ...
        for amount in trading_amounts:
            ...
            risk_percent = 0.05 if abs(sentiment_score) >= 0.7 else 0.03 if abs(sentiment_score) >= 0.5 else 0.02
            ...
            position_recommendations[f"${amount}"] = {
                ...
                "risk_reward_ratio": round(potential_gain / potential_loss, 2) if potential_loss > 0 else 0,
        ...
        return {
            ...
            "sentiment_score": sentiment_score,
            "position_size": position_recommendations.get("$1000", {}).get("contracts", 1),  # Default position size for execute_trade
            ...
            "day_trading_notes": self._generate_day_trading_notes(sentiment_score, confidence, strategy_type)
        }
    
    def _generate_entry_strategy(self, sentiment_score: float, confidence: float, current_price: float) -> Dict:
    ...
    def _generate_day_trading_notes(self, sentiment_score: float, confidence: float, strategy_type: str) -> List[str]:
    ...
    def get_recommendation(self, symbol: str, price_data: Dict, sentiment_data: Dict, signal_data: Dict) -> Dict:
        """
        ...
        recommendation = {
            ...
            "confidence": sentiment_data.get('confidence', 0.5),
            "sentiment_score": sentiment_data.get('sentiment_score', 0),
            "reasoning": trade_signal.get('reasoning', 'Based on sentiment and technical analysis'),
