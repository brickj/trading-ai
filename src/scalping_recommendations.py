...
def get_scalping_candidates():
    ...
    for ticker in market_data:
        ...
        price_change_pct = abs((ticker['current_price'] - ticker['open_price']) / ticker['open_price']) * 100
...
def analyze_sentiment(ticker):
    ...
    for article in headlines:
        ...
        sentiment_scores.append(1 if sentiment['label'] == 'POSITIVE' else -1 if sentiment['label'] == 'NEGATIVE' else 0)

    total_score = sum(sentiment_scores)
    sentiment = "Bullish" if total_score > 2 else "Bearish" if total_score < -2 else "Neutral"

    return {
        "sentiment": sentiment,
        "top_headlines": [{"title": article['title'], "sentiment": sentiment_analyzer(article['title'])[0]['label']} for article in headlines]
    }
