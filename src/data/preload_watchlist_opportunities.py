    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
        logger.info(
            f"[PRELOAD_WATCHLIST_OPPS] Ensured table {WATCHLIST_OPPORTUNITIES_TABLE} exists"
        )
...
def preload_watchlist_opportunities():
    ...
    try:
        logger.info(
            "[PRELOAD_WATCHLIST_OPPS] Starting preload_watchlist_opportunities job..."
        )
        print(
            "[PRELOAD_WATCHLIST_OPPS] Starting preload_watchlist_opportunities job..."
        )
        ...
        logger.info(
            f"[PRELOAD_WATCHLIST_OPPS] Processing {len(watchlist_symbols)} watchlist symbols: {watchlist_symbols}"
        )
        print(
            f"[PRELOAD_WATCHLIST_OPPS] Processing {len(watchlist_symbols)} watchlist symbols: {watchlist_symbols}"
        )
        ...
        logger.info(
            f"[PRELOAD_WATCHLIST_OPPS] Analysis completed: {len(opportunities)} opportunities, {len(errors)} errors"
        )
        print(
            f"[PRELOAD_WATCHLIST_OPPS] Analysis completed: {len(opportunities)} opportunities, {len(errors)} errors"
        )
        ...
        if opportunities:
            logger.info(
                f"[PRELOAD_WATCHLIST_OPPS] Sample opportunity: {opportunities[0]}"
            )
        ...
        logger.info(
            f"[PRELOAD_WATCHLIST_OPPS] Successfully preloaded {len(opportunities)} watchlist opportunities at {timestamp}"
        )
        print(
            f"[PRELOAD_WATCHLIST_OPPS] Successfully preloaded {len(opportunities)} watchlist opportunities at {timestamp}"
        )
        ...
        logger.error(
            f"[PRELOAD_WATCHLIST_OPPS] Failed to preload watchlist opportunities: {e}"
        )
        print(
            f"[PRELOAD_WATCHLIST_OPPS] Failed to preload watchlist opportunities: {e}"
        )
...
def get_latest_preloaded_watchlist_opportunities():
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            with conn.cursor() as cur:
                logger.debug(
                    "[PRELOAD_WATCHLIST_OPPS] Querying for latest watchlist opportunities"
                )
                cur.execute("""
                    SELECT 
                        timestamp,
                ...
                if result:
                    ...
                    if opportunities is None:
                        opportunities = []
                        logger.warning(
                            "[PRELOAD_WATCHLIST_OPPS] No opportunities found in database record"
                        )
                        ...
                        try:
                            ...
                            if not isinstance(opportunities, list):
                                logger.error(
                                    f"[PRELOAD_WATCHLIST_OPPS] Parsed opportunities is not a list: {type(opportunities)}"
                                )
                                opportunities = []
                        except json.JSONDecodeError as je:
                            logger.error(
                                f"[PRELOAD_WATCHLIST_OPPS] Failed to parse opportunities JSON: {str(je)}"
                            )
                            opportunities = []
                    ...
                    try:
                        symbols_analyzed = (
                            int(symbols_analyzed) if symbols_analyzed is not None else 0
                        )
                        errors_count = (
                            int(errors_count) if errors_count is not None else 0
                        )
                    except (ValueError, TypeError) as ve:
                        logger.error(
                            f"[PRELOAD_WATCHLIST_OPPS] Invalid count values: {ve}"
                        )
                        symbols_analyzed = 0
                        errors_count = 0
                    ...
                    logger.warning(
                        "[PRELOAD_WATCHLIST_OPPS] No watchlist opportunities found in database"
                    )
                    return {
                        **default_response,
                        "error": "No watchlist opportunities found",
                    }
