...
def get_scheduler_status():
    """Get scheduler status information for system monitoring"""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT job_name, run_time, enabled, last_run, created_at 
                    FROM job_schedules 
                    ORDER BY run_time
                    """
                )
                ...
                job_list = []
                for job_name, run_time, enabled, last_run, created_at in jobs:
                    job_list.append({
                        "job_name": job_name,
                        "run_time": str(run_time) if run_time else None,
                        "enabled": enabled,
                        "last_run": last_run.isoformat() if last_run else None,
                        "created_at": created_at.isoformat() if created_at else None
                    })

                enabled_count = sum(1 for job in job_list if job["enabled"])
