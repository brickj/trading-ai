...
def setup_default_jobs():
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                for job in default_jobs:
                    ...
                    print(f"✅ Configured job: {job['job_name']} at {job['run_time']} ({job['description']})")
        ...
        print("\n💡 Jobs will run Monday-Friday on trading days.")
...
def verify_job_setup():
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT job_name, run_time, enabled FROM job_schedules ORDER BY run_time")
                jobs = cur.fetchall()
