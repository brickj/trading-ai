...
def scalping_signals_page():
    ...
    try:
        ...
        log_error(
            f"[SCALPING] Error loading scalping signals: {e}\n{traceback.format_exc()}"
        )
...
def get_scalping_opportunities():
    ...
    try:
        ...
        log_info(
            f"[SCALPING] Returning {len(result.get('data', []))} opportunities, total_signals={result.get('total_signals')}, opportunities={result.get('opportunities')}"
        )
...
def run_scalping_analysis():
    ...
    try:
        ...
        log_info(
            f"[SCALPING] Analysis completed. Found {len(opportunities)} opportunities."
        )
        return jsonify(
            {
                "success": True,
...
def get_scalping_history():
    ...
    try:
        ...
        log_info(
            f"[SCALPING] GET /api/scalping/history called with days={days}, limit={limit}"
        )
        query = """
        SELECT ticker, asset_type, date, time_collected, price_open, price_now,
               volume_ratio, price_change_pct, gap_pct, sentiment_class, 
...
def get_opportunities_by_type():
    ...
    try:
        ...
        log_info(
            f"[SCALPING] GET /api/scalping/opportunities_by_type called with type={asset_type}, recommendation={recommendation}"
        )
        query = """
        SELECT ticker, asset_type, date, time_collected, price_open, price_now,
               volume_ratio, price_change_pct, gap_pct, sentiment_class, 
...
def get_scalping_stats():
    ...
    try:
        ...
        log_info(
            f"[SCALPING] Returning stats: today={today_stats}, weekly={weekly_stats}"
        )
        return jsonify(
            {
                "success": True,
