...
class ScalpingAnalyzer:
        ...
        try:
            ...
            if result and len(result) > 0:
                if isinstance(result[0], dict) and result[0].get('exists', False):
                    ...
                    return True
                elif isinstance(result[0], (tuple, list)) and len(result[0]) > 0 and result[0][0]:
                    log_info("Scalping tables already exist")
                    return True
            ...
            for article in news_articles[:5]:  # Top 5 headlines
                if isinstance(article, dict):
                    ...
                    if headline:
                        headlines.append({
                            "title": headline,
                            "sentiment": "Positive" if sentiment_result.get('score', 0) > 0 else "Negative" if sentiment_result.get('score', 0) < 0 else "Neutral"
                        })
    ...
    def generate_scalping_recommendation(self, market_data: Dict, sentiment_data: Dict) -> str:
        ...
        try:
            ...
            if volume_ratio >= self.VOLUME_RATIO_THRESHOLD and abs(price_change_pct) >= self.PRICE_CHANGE_THRESHOLD:
    ...
    def run_morning_scalping_analysis(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            log_info(f"Completed scalping analysis. Found {len(opportunities)} opportunities.")
                            ...
                            for headline_item in headlines_data:
                                if isinstance(headline_item, dict):
                                    signal['top_headlines'].append({
                                        'title': headline_item.get('headline', headline_item.get('title', '')),
                                        'sentiment': headline_item.get('sentiment', 'neutral')
                                    })
                        except Exception as e:
                            print(f"Error parsing headlines for {signal.get('ticker')}: {e}")
                            signal['headlines'] = []
                            signal['top_headlines'] = []
                    else:
                        signal['headlines'] = []
    ...
    def get_scalping_opportunities_api(self) -> Dict[str, Any]:
        ...
        try:
            ...
            log_info(f"[SCALPING] Converted datetime objects, opportunities count: {len(serializable_opportunities)}")
